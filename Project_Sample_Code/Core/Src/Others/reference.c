/**
 * STM32 ↔ MP5475GU PMIC I²C DMA 통신 및 DTC 매핑 구현
 * HAL_I2C_Mem_Read_DMA 사용
 */
...
// ========== PMIC I2C 설정 ==========
#define MP5475_I2C_ADDRESS          0x68    // 7비트 주소
#define MP5475_REG_READ_SIZE        1       // 레지스터 읽기 크기
...
extern uint8_t current_reg_index;
extern uint8_t pmic_reg_address;
extern uint8_t pmic_reg_data;
...
static HAL_StatusTypeDef start_pmic_register_read(uint8_t reg_addr)
{
    ...
    // - hi2c1: I2C1 핸들러
    // - (MP5475_I2C_ADDRESS << 1): 8비트 주소로 변환
    // - reg_addr: PMIC 내부 레지스터 주소
    // - I2C_MEMADD_SIZE_8BIT: 레지스터 주소 크기
    // - i2c_rx_buffer: DMA 데이터 저장 버퍼
    // - 1: 읽을 데이터 크기
